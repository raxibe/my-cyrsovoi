...
fun RepairReturnDocument(
    ...
    val repairReturnDocuments by repairReturnDocumentViewModel.repairReturnDocuments.collectAsStateWithLifecycle()
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Документы возврата с ремонта") },
                navigationIcon = {
        ...
        Box(
            ...
            if (sortedRepairReturnDocuments.isEmpty()) {
                Box(
                    ...
                    Text("Нет документов возврата с ремонта")
                }
            } else {
                LazyColumn(
                    modifier = Modifier.fillMaxSize()
                    ...
                    items(sortedRepairReturnDocuments) { document ->
                        ...
                        val repairType = repairTypes.find { it.id == repairDocument?.repairTypeId }

                        RepairReturnDocumentCard(
                            repairReturnDocument = document,
                            ...
                            onDeleteClick = { repairReturnDocumentViewModel.deleteRepairReturnDocument(document) }
                        )
                    }
                }
            }
...
@Composable
fun RepairReturnDocumentCard(

    repairReturnDocument: RepairReturnDocument,
    repairDocument: RepairDocument?,
    ...
    val formattedDate = remember(repairReturnDocument.creationDate) {
        SimpleDateFormat("dd.MM.yyyy HH:mm", Locale.getDefault())
            .format(Date(repairReturnDocument.creationDate))
    }
        ...
        Box(
            ...
            Row(
                ...
                Column(
                    ...
                    Text(
                        text = "Документ возврата №${repairReturnDocument.documentNumber}",
                    ...
                    Text(
                        text = "Состояние оборудования: ${repairReturnDocument.condition}",
                        style = MaterialTheme.typography.bodyMedium
                    )
                }
